...
async function runTurn({ cfg, pack, question, history, onEvent }) {
  ...
  const generate = (attempt) => {
    ...
    return ollama.chatStream(cfg, messages, {}, (t) => emit({ type: "gen_token", text: t }));
  };
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (result.ok) {
      ...
      const summary = await summarize(cfg, question, result, emit).catch(() => null);
      return { kind: "sql", attempts, result, summary, meta };
    }
    ...
    if (attempt === maxRetries) {
      return { kind: "sql", attempts, result: null, summary: null, meta };
    }
...
async function summarize(cfg, question, result, emit = () => {}) {
  ...
  const text = await ollama.chatStream(cfg, [{ role: "user", content }], { temperature: 0.2 },
